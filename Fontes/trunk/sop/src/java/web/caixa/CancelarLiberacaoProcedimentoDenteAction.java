 ...
 */
public class CancelarLiberacaoProcedimentoDenteAction extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        if (boca == null) {
            System.out.println("Registro de Boca (ID = " + idBoca + ") NAO encontrado em CancelarLiberacaoProcedimentoDenteAction");
        ...
        if (ficha == null) {
            System.out.println("Registro de Ficha do paciente (ID = " + boca.getPaciente().getId() + ") NAO encontrado em CancelarLiberacaoProcedimentoDenteAction");
            return mapping.findForward(FALHACONSULTAR);
        }
        ...
        if (historicoDente == null) {
            System.out.println("Registro de HistoricoDente (ID = " + idHistoricoDente + ") NAO encontrado em CancelarLiberacaoProcedimentoDenteAction");
            return mapping.findForward(FALHACONSULTAR);
        }

        // Verifica se o registro a ser excluido nao estah com status liberado (pago)
        if (!historicoDente.getStatusProcedimento().equals(StatusProcedimentoService.getStatusProcedimentoPago())) {
            ...
            // Credita saldo com o valor do tratamento...
            ficha.setSaldo(ficha.getSaldo() + historicoDente.getValorCobrado());
            if (FichaService.atualizar(ficha)) {
                System.out.println("Registro de Ficha (ID = " + ficha.getId() + ") atualizado com sucesso em CancelarLiberacaoProcedimentoDenteAction");
            }
            else {
                System.out.println("Falha ao atualizar registro de Ficha (ID = " + ficha.getId() + ") em CancelarLiberacaoProcedimentoDenteAction");
            ...
            if (!HistoricoDenteService.atualizar(historicoDente)) {
                System.out.println("Falha ao atualizar registro de historicoDente (ID = " + idHistoricoDente + ") em CancelarLiberacaoProcedimentoDenteAction");
                return mapping.findForward(FALHAATUALIZAR);
            }
            else {
                ...
                if (!DenteService.atualizar(denteAux2)) {
                    System.out.println("Falha ao atualizar dente (ID = " + denteAux2.getId() + ") em CancelarLiberacaoProcedimentoDenteAction");
                    return mapping.findForward(FALHAATUALIZAR);
                }
                else {
