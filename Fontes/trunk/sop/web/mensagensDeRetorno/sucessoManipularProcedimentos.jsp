<%-- 
    Document   : sucessoManipularProcedimentos
    Created on : 20/06/2010, 14:46:08
...
<%
    ...
    // Captura o id do paciente colocado na sessao por AtenderAtendimentoTratamentoAction, ContinuarAtendimentoTratamentoAction ou VisualizarHistoricoBocaAction
    String idPaciente = "";
...
<html>
    <head>
        ...
        %>
        <meta http-equiv="refresh" content="1;url=<%=request.getContextPath()%>/app/filaAtendimentoTratamento/actionContinuarAtendimentoTratamento.do?idPaciente=<%=idPaciente%>">
        <%
            } else if (!idDente.isEmpty() && tipo.equals("dente")) {
        %>
        <meta http-equiv="refresh" content="1;url=<%=request.getContextPath()%>/app/atendimentoTratamento/manipularProcedimentos.do?idBoca=<%=idBoca%>&idDente=<%=idDente%>">
        <%
            } else if (!idBoca.isEmpty() && tipo.equals("boca")) {
        %>
        <meta http-equiv="refresh" content="1;url=<%=request.getContextPath()%>/app/atendimentoTratamento/manipularProcedimentosBoca.do?idPaciente=<%=idPaciente%>">
        <%
            }
        %>
