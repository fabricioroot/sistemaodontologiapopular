<%-- 
    Document   : manipularProcedimentosBoca
    Created on : 17/06/2010, 22:05:21
...
<html>
    <head>
        ...
        <title>Manipular Procedimentos / Boca Completa - SOP - Sistema de Odontologia Popular</title>
    </head>
    <body onload="goFocus('procedimentoId')">
        <jsp:include page="/app/cabecalho.jsp" flush="true">
            <jsp:param name="titulo" value="Manipular Procedimentos / Boca Completa"/>
        </jsp:include>
        <br> &nbsp;
        <br> &nbsp;
        ...
        <%
            ...
            // Este objeto vem do action VisualizarHistoricoBocaAction acionado por botao da pagina atenderAtendimentoTratamento
        ...
        <table cellpadding="1" cellspacing="1" align="center" width="100%">
                        ...
                        <tr>
                            ...
                            <td style="border:none" align="center" valign="middle">
                                <%
                                    if (orcado) {
                                %>
                                    <input name="excluirRegistroHistoricoBoca" class="botaodefault" type="button" value="excluir" title="Excluir" onclick="javascript: if (confirm('Tem certeza que deseja EXCLUIR este registro?')) location.href='<%=request.getContextPath()%>/app/atendimentoTratamento/excluirRegistroHistoricoBoca.do?idHistoricoBoca=${idHistoricoBoca}'" onMouseOver="javascript:this.style.background='url(<%=request.getContextPath()%>/imagens/bgbt-med-over.png)'" onMouseOut="javascript:this.style.background='url(<%=request.getContextPath()%>/imagens/bgbt-med.png)'">
                                <%
                                    } // if (orcado)
                                    else
                                    if (liberado) {
                                %>
                                    <input name="iniciarProcedimento" class="botaodefault" type="button" value="iniciar procedimento" title="Iniciar" onclick="javascript: if (confirm('Tem certeza que deseja INICIAR este procedimento?')) location.href='<%=request.getContextPath()%>/app/atendimentoTratamento/iniciarProcedimentoBoca.do?idHistoricoBoca=${idHistoricoBoca}'" onMouseOver="javascript:this.style.background='url(<%=request.getContextPath()%>/imagens/bgbt-med-over.png)'" onMouseOut="javascript:this.style.background='url(<%=request.getContextPath()%>/imagens/bgbt-med.png)'">
                                <%
                                    } // if (liberado)
                                    else
                                    if (emTratamento) {
                                %>
                                    <input name="finalizarProcedimento" class="botaodefaultextrag" type="button" value="finalizar procedimento" title="Finalizar" onclick="javascript: if (confirm('Tem certeza que deseja FINALIZAR este procedimento?')) location.href='<%=request.getContextPath()%>/app/atendimentoTratamento/finalizarProcedimentoBoca.do?idHistoricoBoca=${idHistoricoBoca}'" onMouseOver="javascript:this.style.background='url(<%=request.getContextPath()%>/imagens/bgbt-extrag-over.png)'" onMouseOut="javascript:this.style.background='url(<%=request.getContextPath()%>/imagens/bgbt-extrag.png)'">
                                <%
                                    } // if (emTratamento)
                                %>
                            </td>
            ...
            <tr>
                <td colspan="4">
                    <form name="formIncluirProcedimento" id="formIncluirProcedimento" method="POST" onsubmit="return validarDadosBoca()" action="<%=request.getContextPath()%>/app/atendimentoTratamento/actionIncluirProcedimentoBoca.do">
